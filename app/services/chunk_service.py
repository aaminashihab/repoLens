...
class ChunkService:
    ...
    def _extract_jsts_chunks(self, source: bytes, file_path: str) -> list[CodeChunk]:
        """Extract symbol-level chunks for JavaScript and TypeScript files using regex patterns."""
        chunks: list[CodeChunk] = []
        content_str = source.decode("utf-8", errors="replace")
        lines = content_str.splitlines()
        if not lines:
        ...
        # Regex patterns for JS/TS function, class, and method definitions
        patterns = [
            ...
            (re.compile(r"^\s*(?:async\s+)?([A-Za-z0-9_$]+)\s*\(.*?\)\s*\{"), "method"),
        ]

        symbol_positions: list[tuple[int, str, str]] = []
        for idx, line in enumerate(lines):
            for pattern, sym_type in patterns:
                match = pattern.search(line)
                if match:
                    sym_name = match.group(1)
                    if sym_name not in {"if", "for", "while", "switch", "catch", "constructor"}:
                        symbol_positions.append((idx, sym_name, sym_type))
                        break

        if not symbol_positions:
            return self._extract_generic_chunks(source, file_path)

        for i, (start_idx, name, sym_type) in enumerate(symbol_positions):
            end_idx = symbol_positions[i + 1][0] if i + 1 < len(symbol_positions) else min(len(lines), start_idx + 60)
    ...
    def _extract_jsts_graph_edges(
        self, source: bytes, file_path: str, graph: RepositoryGraph
    ) -> None:
