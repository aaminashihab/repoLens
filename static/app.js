...
async function checkHealth() {
    ...
    const setState = (state) => {
        ...
        if (label)
            label.textContent = state === "ok" ? "Connected" : state === "error" ? "Offline" : "Checking…";
    };
    setState("checking");
    try {
...
function setActiveIndex(indexId, repoUrl, chunkCount) {
    ...
    if (chatInput) {
        ...
        chatInput.placeholder = "Enter a claim to verify…";
    }
    if (sendBtn)
...
async function deleteIndex(indexId) {
    ...
    if (!dialog) {
        if (!confirm("Delete this index? This cannot be undone."))
            return;
...
async function doDelete(indexId) {
    try {
        ...
        if (res.ok) {
            if (currentIndexId === indexId) {
                ...
                if (chatInput) {
                    ...
                    chatInput.placeholder = "Select an index to verify claims…";
                }
                if (sendBtn)
...
// ─── Indexing ─────────────────────────────────────────────────────────────────
function setIndexingState(processing, message, isError = false) {
...
async function pollStatus(indexId, repoUrl, retries = 0) {
    const MAX_RETRIES = 5;
    try {
        ...
        if (data.status === "processing") {
            ...
            const allIndexes = listRes.ok ? await listRes.json() : [];
            const entry = allIndexes.find(i => i.index_id === indexId);
            if (entry)
                setActiveIndex(indexId, repoUrl, entry.vector_count);
        }
...
function renderOnboardingGuide(container) {
    container.innerHTML = `
...
function renderVerificationReport(report, container) {
    const statusConfig = {
...
async function handleVerification(claim) {
    ...
    const contentEl = responseEl.querySelector(".message__content");
    ...
    try {
        ...
        if (res.status === 422) {
            ...
            const isClaimError = Array.isArray(detail) && detail.some((d) => d.loc?.includes("claim"));
            if (isClaimError) {
...
document.addEventListener("keydown", (e) => {
    if ((e.ctrlKey || e.metaKey) && e.key === "k") {
        ...
        if (chatInput && !chatInput.disabled)
            ...
            showToast("Select an index to start verifying", "info");
    }
    if (e.key === "Escape") {
        const chatInput = document.getElementById("chat-input");
